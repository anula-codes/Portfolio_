...
// Theme presets — tweak these hex values to match your design system
const THEME_PRESETS = {
  dark: {
    waveColor: [0.2, 0.35, 0.6],   // muted blue-indigo wave
    bgColor:   [0.04, 0.04, 0.06], // near-black background
  },
  light: {
    waveColor: [0.55, 0.45, 0.75], // soft lavender wave
    bgColor:   [0.94, 0.93, 0.9],  // warm off-white background
  },
};

export default function Dither({
  // Pass 'dark' | 'light', or omit to auto-detect from prefers-color-scheme
  theme,
  // Override individual colors if you don't want the preset
  waveColor,
  bgColor,
  // Standard props
  waveSpeed = 0.05,
  ...
  // Resolve theme: explicit prop > system preference > dark fallback
  const resolvedTheme =
    theme ??
  ...
  const preset = THEME_PRESETS[resolvedTheme] ?? THEME_PRESETS.dark;
  const resolvedWaveColor = waveColor ?? preset.waveColor;
